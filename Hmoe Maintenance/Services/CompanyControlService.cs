...
namespace Hmoe_Maintenance.Services
{
    public class CompanyControlService : ICompanyControlService
    {
        ...
        public CompanyControlService(AppDBcontext context)
        {
            _Context = context;
        }

        public async Task<List<Notification>> GetAllNotificationToCompany(string userid)
        {
            var allNotifications = await _Context.Notification.Where(e=>e.UserId == userid && e.IsRead == false)
        ...
        public async Task<bool> ApprovecompanyRequest(int notifid)
        {
            ...
            var requestman = await _Context.MaintenanceRequests.Include(e => e.Customer).FirstOrDefaultAsync(e => e.RequestNumber == notification.RelatedEntityId );
        ...
        public async Task<bool> AssignedTechnicianRequest(int id, int Tecid)
        {
            ...
            var Techniciancopy = await _Context.TechnicianProfileCopies.FirstOrDefaultAsync(e => e.Id == Tecid && e.IsAvailable);
